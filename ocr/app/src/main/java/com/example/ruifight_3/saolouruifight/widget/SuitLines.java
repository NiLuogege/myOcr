...
/**
 * Created by RuiFight-3 on 2019/3/22.
...
public class SuitLines extends View {
    ...
    private void drawClickHint(Canvas canvas) {
        ...
        canvas.drawText("录入量 : " + cur.getValue(), bak.centerX(),
    ...
    private void calcMaxUnit(Map<Integer, List<Unit>> datas) {
        ...
        //防止最大数据也是0 的情况下 出现绘制出错
        if (Util.getCeil5(Math.max(maxUnit.getValue(), 0)) == 0.0) {
            minAndMaxOfY[1] = 5;
        } else {
