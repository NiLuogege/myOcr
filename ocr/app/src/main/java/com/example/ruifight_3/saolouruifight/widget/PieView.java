 ...
 * Describe: 自定义饼状图  由于数据小 文字会重叠 问题  放弃使用   暂时保留  2019  4  22
 */
public class PieView extends View {
    ...
    public void setDate(List<PieEntry> list){
        this.list = list;
        if (mSweep == null){
    ...
    private void initPie(Canvas canvas) {
        ...
        if (list != null && anglesSize > 0){
            ...
            for (int i = 0;i < anglesSize; i++){
                piePaint.setColor(ContextCompat.getColor(context,list.get(i).getColor()));
